...
NetworkUpdate.Init = function () {
	...
	$("#selIdcCode").change(function () {
		NetworkUpdate.GetRackInfo($(this).val());
	});

    $("#btnSave").click(NetworkUpdate.Update);
    $("#btnClose").click(NetworkUpdate.Close);
}

NetworkUpdate.Update = function () {
    ...
    $.ajax({
        type: "POST",
        url: "/Asset/SetNetworkUpdate/",
        data: data,
        dataType: "JSON",
        success: function (result) {
            if (result != null && result.statusCode == 0) {
                alert('저장되었습니다.');
                opener.NetworkList.Search();
                NetworkUpdate.Close();
            }
        },
...
NetworkUpdate.Close = function () {
	window.close();
}

//RACK 리스트 조회
NetworkUpdate.GetRackInfo = function (IdcCode) {
